...
class AdminFunctions:
    ...
    def mark_book_as_lost(self, book_id):
        cursor = self.db_connection.cursor()
        try:
            cursor.execute("UPDATE book SET CurrentState = 'Lost' WHERE book_id = %s", (book_id,))
            self.db_connection.commit()
            q="DELETE FROM borrowed_books WHERE book_id=%s"
            cursor.execute(q, (book_id,))
            self.db_connection.commit()
            return True, f"Book {book_id} has been marked as lost."
        except Exception as e:
            self.db_connection.rollback()
            return False, str(e)
        finally:
